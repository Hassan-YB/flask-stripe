...
def payment():
    ...
    return render_template ('payment.html', name='', profile='', payAmount= my_item['price'], currency= my_item['currency'])
...
def success():
    ...
    if my_item is None:
        logWebApp.error("User " + " " + " paid successfully product " + session['productId'] + ". Anyway wsgi couldn't retrieive it for updating limitsConfiguration collection")
    ...
    #####################################################################################
    ######## SETTING USER LIMITS, TEMPORARY COMMENTED REPORTING           ###############
    #####################################################################################
    '''    
    try:
